#!/bin/bash
#-----------------------------------------------------------------------
# This script runs on a cloud-based Ubuntu linux machine
# with an apache server running.
# This script creates an HTML file and copies it to the directory
# served by apache.
#
# Prereqs:
#	- Runs as user 'pi' on the cloud machine.
#
# Setup: 
...
#
# Consumption:
#	Browse to the HTML file to view status of this machine.
#	The browser auto-updates every minute (via meta refresh)
#
# AD 2019-0607-2258 Copyright BMIR 2019
#-----------------------------------------------------------------------
export DELIMITER="----------------------------------------------------------------------------------"

export OUTFILE="bmir.cloud.status.html"
###ort REMOTE_USER="pi"
###ort REMOTE_SERVER="dobmir"
export REMOTE_FOLDER="/home/pi/bmir"
export LOCAL_FOLDER="/home/pi/bmir"
export LOCAL_FILESYSTEM="/dev/root"
export LOCAL_USER="pi"
...
echo "<BODY>" >> ${OUTFILE}
echo "<TITLE>BMIR Cloud System Status</TITLE>" >> ${OUTFILE}
echo "<H3>BMIR Cloud System Status</H3>" >> ${OUTFILE}
echo "<a href=\"./\">Parent Directory</a><br>" >> ${OUTFILE}
echo "<a href=\"bmir.archiver.status.html\">bmir.archiver.status.html</a><br>" >> ${OUTFILE}
...
echo "BMIR Cloud System Status" >> ${OUTFILE}
date >> ${OUTFILE}
hostname >> ${OUTFILE}
uptime >> ${OUTFILE}
...
ps -ef | grep apache | grep -v grep >> ${OUTFILE}
ps -ef | grep ssh | grep -v grep >> ${OUTFILE}
...
ls -lrt ${LOCAL_FOLDER}/${DATE} >> ${OUTFILE}
...
# Copy HTML
cp -p ${OUTFILE} ${REMOTE_FOLDER}/${OUTFILE}
rc=$?
if [ 0 != ${rc} ] ; then
	echo "ERROR ${rc} could not copy output file."
	exit 1
fi
